...
# Concurrency (p>1): read TSV (url<tab>name) and call function directly in background; PID throttle.
_run_queue_tsv() {
  local -a pids=()
...
# Build the list of pairs (url,name) from the target file
_build_pairs_from_file() {
  local tf="$1"; local_name=""; local count=0
  queue_tmp=$(mktemp "/tmp/curl_fetcher.queue.XXXXXX")
  while IFS= read -r raw; do
    # Remove trailing CR if present (Windows files) and trim
    line="${raw%%$'\r'}"
    [[ -z "$line" ]] && continue
    case "$line" in
      \#*) continue ;;
...
if [[ "$MODE" == "single" ]]; then
  ...
  pair_count=$(_build_pairs_from_file "$TARGET_FILE")
  if (( pair_count == 0 )); then
    _err "No valid 'filename:<name>' + 'https…' pairs found in $TARGET_FILE"; exit 1
  fi
  if (( MAX_PARALLEL == 1 )); then
    # Pure serial execution — avoids any concurrency machinery
    while IFS=$'\t' read -r url name; do
      [[ -z "$url" || -z "$name" ]] && continue
      _download_one "$url" "$name"
    done < "$queue_tmp"
...
# Version 1.3 — Per-job temp dirs; PID-throttle; consistent behavior regardless of -d.
# Version 1.4 — Serial path when -p 1 (default). Concurrency machinery only used
#               for -p > 1. Added explicit 'no pairs found' message for -f mode.

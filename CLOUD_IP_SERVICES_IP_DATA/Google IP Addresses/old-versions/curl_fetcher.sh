...
# Concurrency without subshell bash -c: run functions directly in background
_run_queue() {
  local -a CMDS=()
  while IFS=$'\n' read -r line; do [[ -n "$line" ]] && CMDS+=("$line"); done
  local pids=()
  for cmd in "${CMDS[@]}"; do
    # throttle
    while true; do
...
if [[ "$MODE" == "single" ]]; then
  ...
  # Call directly (no queue needed unless user sets -p>1, but single item is fine)
  ...
  while IFS= read -r raw; do
    ...
    case "$line" in
      ...
      https*)
        ...
        printf '%s\n' "_download_one \"$line\" \"$local_name\"" >> "$queue_tmp"
        local_name=""
        ;;
      *) ;; # ignore others
    esac
  done < "$TARGET_FILE"
  _run_queue < "$queue_tmp"
  rm -f "$queue_tmp"
fi
...
# ===== VERSION HISTORY =====
# Version 1.0 — Initial creation of the script: parsing, curl -O downloads, rename,
# debug (-d), concurrency (-p 1..5), single-target (-t/--tr with -fn), help (-h),
# version (-v), Ctrl+C termination.
# Version 1.1 — Fixed: single-target and parallel execution now call functions
# directly without 'bash -c', preventing 'command not found' for _download_one
# on macOS/Linux.

...
# Build pairs and return queue path + count as 'path:count'
_make_queue_from_file() {
  local tf="$1"
  local q c=0 local_name="" line
  q=$(mktemp "/tmp/curl_fetcher.queue.XXXXXX") || { _err "mktemp failed"; return 1; }
...
if [[ "$MODE" == "single" ]]; then
  ...
  qret=$(_make_queue_from_file "$TARGET_FILE") || { _err "Failed to parse $TARGET_FILE"; exit 1; }
  queue_tmp=${qret%%:*}
  pair_count=${qret##*:}
  if [[ -z "$queue_tmp" || ! -s "$queue_tmp" || ! "$pair_count" =~ ^[0-9]+$ ]]; then
    _err "No valid 'filename:<name>' + 'https…' pairs found in $TARGET_FILE"; rm -f "$queue_tmp" 2>/dev/null; exit 1
  fi
...
# Version 1.5 — Fixed empty queue_tmp by returning 'path:count' from parser and
#               guarding reads; works reliably in non-debug runs and with spaces.

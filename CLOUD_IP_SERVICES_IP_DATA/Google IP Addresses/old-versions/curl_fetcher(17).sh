...
Options:
  ...
  -ck           Check-only: confirm endpoint is up.
                • URLs: HEAD then GET fallback.
                • Bare IPv4: try http:// then https://; report up|down per IP.
...
In -ck (check-only) mode, lines starting with 'http(s)' are treated as URLs; bare IPv4
lines are validated and checked via http then https.
USAGE
}
...
# Curl attempt wrapper with retries and timeout (for check mode)
_curl_with_retry() {
  cmd="$1"
  attempt=1
...
_run_parallel_check() {
  ...
  while (( i < total )); do
    ...
    [[ "$STOP_REQUESTED" == true ]] && break
    # spawn a lightweight bash -c per check item to avoid scope concerns
    bash -c '
      url="$1"; rt="$2"; to="$3"; report="$4"; 
      # simple wrapper: call the main checker via a single bash -c is not ideal, but we
      # keep logic here for portability; however, to reuse code we just exec parent shell function
      # by re-invoking this script with -ck -t url (ensures same behavior).
    ' _ "${URLS[i]}" "$RT_COUNT" "$TO_SEC" "$CK_REPORT" &
    # Actually call in-shell checker (it is safe and in-scope here)
    _check_one "${URLS[i]}" &
    pids+=($!)
    ((i++))
  done
...
# Version 2.7.2 — Robust parallel download workers: spawn dedicated bash processes
#                 per item using passed args; avoids function-scope issues causing
#                 "_download_one: command not found" on some shells.

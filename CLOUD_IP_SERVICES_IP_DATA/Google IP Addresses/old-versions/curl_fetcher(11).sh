...
TS_ENABLED=false
TS_SUFFIX=""
ORIGINAL_PWD="$(pwd)"  # Capture the directory where the script is invoked
...
Options:
  ...
  -dr <dir>     Output directory. If relative, it is resolved against the directory
                you run the script from. Created if missing; results saved inside it.
...
# If user provided a relative path, anchor it to ORIGINAL_PWD
case "$OUTPUT_DIR" in
...
# Version 2.0 — Added -dr <dir> and -ts (week-based timestamp).
# Version 2.1 — Changed -ts to _<DD-MM-YYYY_HH-MM>.
# Version 2.2 — -dr now resolves relative paths against the invocation directory,
#               ensuring moves from per-job temp dirs always target the intended
#               output folder.
